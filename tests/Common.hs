...
instance FromJSON Test where
  parseJSON = withObject expected p
    where p o      = if length (M.keys o) == 3 || length (M.keys o) == 4
                     then Test <$> o .: "name" <*> o .: "code" <*> o .: "result" <*> o .:? "anf" .!= False
                     else typeMismatch expected (Object o)
          expected = "{ \"name\" : ..., \"code\" : ..., \"result\" : ..., \"anf\" : \"False\"}"

createTestTree sc Test{..} = let res = case testResult of
                                         Value   s -> Right s
                                         Failure s -> Left  s
                                 mk = if testAnf then anfTest else mkTest
                             in  mk sc testName testCode res
